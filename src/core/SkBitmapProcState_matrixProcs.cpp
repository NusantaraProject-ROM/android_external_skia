...
SkBitmapProcState::MatrixProc SkBitmapProcState::chooseMatrixProc(bool translate_only_matrix) {
    ...
    if( fTileModeX == fTileModeY ) {
        ...
        if (translate_only_matrix && kNone_SkFilterQuality == fFilterQuality) {
            switch (fTileModeX) {
                ...
                case SkShader::kClamp_TileMode:  return  clampx_nofilter_trans<int_clamp>;
                case SkShader::kRepeat_TileMode: return repeatx_nofilter_trans<int_repeat>;
                case SkShader::kMirror_TileMode: return mirrorx_nofilter_trans<int_mirror>;
            }
        }
    ...
    if (fTileModeX == SkShader::kMirror_TileMode) {
        index = index + 4;
    }

    if (fTileModeY == SkShader::kMirror_TileMode ||
        (fTileModeY == SkShader::kRepeat_TileMode && fTileModeX == SkShader::kMirror_TileMode )) {
        index += 1;
    }
